...
function unwrapFencedMarkdown(text) {
  const fenced = text.trim().match(/^```(?:md|markdown)?\s*\n([\s\S]*?)\n```$/i)
  return fenced ? fenced[1] : text
}

function normalizeMarkdown(value = '') {
  return unwrapFencedMarkdown(String(value).replace(/\r\n/g, '\n'))
    .replace(/[ \t]+(#{1,6}\s*)/g, '\n\n$1')
    .replace(/([。；;:：])\s*((?:[-*+]\s*|\d+[.)]\s*))/g, '$1\n$2')
    .replace(/[ \t]+(>\s?)/g, '\n\n$1')
    .trim()
...
function pushBlock(blocks, type, lines) {
...
export function renderMarkdown(value = '') {
  return parseBlocks(value).map((block) => {
